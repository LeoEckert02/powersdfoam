...
def _greedy_min_overlap_fps(sample_points, sample_radii, threshold=0.1):
    """Greedy farthest-point sampling using minimum pairwise overlap.

    Selects a subset of (sample_points, sample_radii) such that no two selected
    spheres overlap by more than ``threshold`` (in the
    (r_i + r_j - ||p_i - p_j||) / (r_i + r_j) sense).

    This is mathematically identical to the original O(K*M) inner loop -- it
    picks the same indices in the same order -- but maintains the running
    ``max_overlap`` array incrementally so each iteration is O(N) instead of
    O(N*K) and allocates no K x M intermediate tensor.
    """
    n = sample_points.shape[0]
    device = sample_points.device
    is_candidate = torch.zeros(n, dtype=torch.bool, device=device)
    if n == 0:
        return is_candidate

    seed = int(torch.randint(0, n, (1,), device=device).item())
    is_candidate[seed] = True

    sum_r = sample_radii + sample_radii[seed]
    dist = (sample_points - sample_points[seed]).norm(dim=-1)
    max_overlap = (sum_r - dist) / sum_r
    max_overlap[seed] = float("inf")  # never re-select an already-picked point

    inf = torch.tensor(float("inf"), device=device, dtype=max_overlap.dtype)
    while True:
        val, idx = torch.min(max_overlap, dim=0)
        if val.item() > threshold:
            break
        idx_int = int(idx.item())
        is_candidate[idx_int] = True
        max_overlap[idx_int] = inf

        sum_r = sample_radii + sample_radii[idx_int]
        dist = (sample_points - sample_points[idx_int]).norm(dim=-1)
        new_overlap = (sum_r - dist) / sum_r
        torch.maximum(max_overlap, new_overlap, out=max_overlap)
...
def get_steiner_points(points, radii, cameras):
    ...
    for i in range(10):
        ...
        # Select candidates from samples so that they have minimum overlap with
        # each other. Uses an incremental O(N^2) update over the original
        # O(N^3) double loop; result is identical.
        is_candidate = _greedy_min_overlap_fps(
            sample_points, sample_radii, threshold=0.1
        )
